function doGet(e) {
  if (e.parameter.action) {
    var res = apiHandler(e.parameter.action, e.parameter);
...
function getAvailableDates() {
  try {
    ...
    // Считаем занятость для ОБЩЕЙ БАЗЫ (только те сплавы, где НЕ выставлен лимит)
    const dailyOccupiedForBase = {};
    for (let i = 1; i < data.length; i++) {
      const details = tripDetails[i-1];
      const dStr = details.dateStr;
      if (!dStr) continue;

      const lim2Val = getVal(data[i], 'Лимит байдарка 2М');
      const lim3Val = getVal(data[i], 'Лимит байдарка 3М');

      if (!dailyOccupiedForBase[dStr]) dailyOccupiedForBase[dStr] = { b2: 0, b3: 0 };
      
      // Если лимит НЕ выставлен (пустая ячейка), значит этот сплав тратит ОБЩУЮ базу
      if (lim2Val === "") dailyOccupiedForBase[dStr].b2 += (details.b2 || 0);
      if (lim3Val === "") dailyOccupiedForBase[dStr].b3 += (details.b3 || 0);
    }
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const base2M = parseInt(getVal(data[i], 'Общий парк 2М')) || 0;
      const base3M = parseInt(getVal(data[i], 'Общий парк 3М')) || 0;
      ...
      const dayOccBase = dailyOccupiedForBase[dateStr] || { b2: 0, b3: 0 }; 

      let remP  = limP >= 0 ? Math.max(0, limP - (details.b2*2 + details.b3*3)) : "";
      
      // Логика Байдарок: Лимит (если есть) ИЛИ Общая База (если лимита нет)
      let rem2M = lim2 >= 0 ? Math.max(0, lim2 - details.b2) : Math.max(0, base2M - dayOccBase.b2);
      let rem3M = lim3 >= 0 ? Math.max(0, lim3 - details.b3) : Math.max(0, base3M - dayOccBase.b3);
...
function processBooking(data) {
  ...
  try {
    ...
    if (!tripSheet) {
      ...
      // Используем setValues вместо appendRow для шапки, чтобы формулы SUM точно заработали
      tripSheet.getRange(1, 1, 1, headerFormulas.length).setValues([headerFormulas]);
      tripSheet.getRange(1, 1, 1, headerFormulas.length).setFontWeight('bold').setBackground('#dcfce7').setBorder(true, true, true, true, true, true);
    ...
    if (checkMulti.found && checkMulti.allCount > 1) {
       ...
       if (data.hasTelegram === 'Нет') {
         tripSheet.getRange(lastRow, 1, 1, 21).setBackground('#ff4d4d');
       } else if (isWaitlist) {
         tripSheet.getRange(lastRow, 1, 1, 21).setBackground('#fb923c');
       }
    }
...
function updateStatistics(dateStr, peopleCount, sum) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (rowDate === targetDateStr) {
        const bookings = (parseInt(data[i][1]) || 0) + 1;
        const people = (parseInt(data[i][2]) || 0) + peopleCount;
        const revenue = (parseFloat(data[i][3]) || 0) + sum;
        const avgCheck = revenue / bookings;

        stats.getRange(i + 1, 2).setValue(bookings).setNumberFormat('0');
        stats.getRange(i + 1, 3).setValue(people).setNumberFormat('0');
        stats.getRange(i + 1, 4).setValue(revenue).setNumberFormat('0');
        stats.getRange(i + 1, 5).setValue(avgCheck).setNumberFormat('0');
        found = true;
        break;
      }
    }
    if (!found) {
      const newRow = [dateStr, 1, peopleCount, sum, sum];
      stats.appendRow(newRow);
      stats.getRange(stats.getLastRow(), 2, 1, 4).setNumberFormat('0');
    }
  } catch (e) {
...
function apiHandler(action, params) {
  if (action === 'getDates') return getAvailableDates();
  if (action === 'processBooking') return processBooking(JSON.parse(decodeURIComponent(params.data)));
  if (action === 'checkExistingBooking') return checkExistingBooking(params.phone, params.tripName);
  if (action === 'cancelBooking') return cancelBooking(params.phone, params.tripName);
